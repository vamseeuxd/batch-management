...
export class Service {
  private action: AngularFirestoreCollection<IData>;
  readonly data$: Observable<IData[]>;
  ...
  constructor(
    ...
    this.action = firestore.collection<IData>(
      ModuleConfig.name_lowercase_plural
    );
    this.data$ = firestore
      .collection<IData>(ModuleConfig.name_lowercase_plural, (ref) => {
        return ref.where('deleted', '==', false).orderBy('createdOn');
      })
      .valueChanges();
  }

  addData(data: IData): Promise<any> {
  ...
  updateData(data: IData): Promise<any> {
    return new Promise(async (resolve, reject) => {
      if (data.name.trim().length > 0) {
        const busyIndicatorId = this.busyIndicator.show();
  ...
  deleteData(data: IData): Promise<any> {
    return new Promise(async (resolve, reject) => {
      const busyIndicatorId = this.busyIndicator.show();
      try {
